...
const cardStyle = $derived(
	resolveComponentStyles('consentDialogCard', theme.theme, { baseClassName: styles.card, noStyle }, noStyle)
);

const headerStyle = $derived(
	resolveComponentStyles('consentDialogHeader', theme.theme, { baseClassName: styles.header, noStyle }, noStyle)
);

const titleStyle = $derived(
	resolveComponentStyles('consentDialogTitle', theme.theme, { baseClassName: styles.title, noStyle }, noStyle)
);

const descriptionStyle = $derived(
	resolveComponentStyles('consentDialogDescription', theme.theme, { baseClassName: styles.description, noStyle }, noStyle)
);

const contentStyle = $derived(
	resolveComponentStyles('consentDialogContent', theme.theme, { baseClassName: styles.content, noStyle }, noStyle)
);
...
>
	<Portal>
		...
		/>
		<Dialog.Positioner
			class={noStyle ? '' : `${styles.root || ''} ${!disableAnimation ? (isOpen ? styles.dialogVisible || '' : styles.dialogHidden || '') : ''}`}
